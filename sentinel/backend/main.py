...
Endpoints:
  ...
  GET  /risk-summary        → top 10 highest risk assets
  GET  /vulnerabilities     → all CVEs across all assets
  GET  /orphans             → assets with no owner
  GET  /stats               → dashboard summary counts
  GET  /ask                 → AI Q&A (RAG pipeline — unchanged)
...
def create_asset(asset_input: AssetInput, db: Session = Depends(get_db)):
    ...
    # Build asset dict in the format predict.py expects
    asset_dict_for_ml = {
        "asset_id":         asset_input.asset_id,
        "asset_type":       asset_input.asset_type,
        "environment":      asset_input.environment,
