...
# ─── AUTH GATE ────────────────────────────────────────────────────────────────
# show_login() must be called AFTER set_page_config.
# login.py does NOT call set_page_config — it only renders the form widgets.
...
def auth_headers() -> dict:
    """Returns the Authorization header using the current session JWT."""
...
# ─── HELPER: fetch stats (authenticated) ─────────────────────────────────────
# /stats requires a valid JWT — we pass auth_headers() in every call.
# ttl=60 caches the result for 60 seconds to avoid hammering the API on
# every widget interaction, but we pass the token as a parameter so
# Streamlit's cache key includes it (different users get different caches).

@st.cache_data(ttl=60)
def fetch_stats(token: str):
    """
    Fetches /stats from the Sentinel API.
    token is passed in so the cache is keyed per-user, not shared globally.
...
# ─── STATS CARDS ─────────────────────────────────────────────────────────────
# Pass the JWT token so the request is authenticated
