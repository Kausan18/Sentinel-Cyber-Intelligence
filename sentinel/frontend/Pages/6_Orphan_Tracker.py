...
def fetch_orphans():
    ...
    try:
        response = requests.get(f"{API}/orphans", timeout=30)
...
def format_orphans_for_table(orphans):
    """Convert orphan asset list into a clean DataFrame."""
    rows = []
    for a in orphans:
...
# Sort by risk score — highest first so most dangerous orphans are at top
