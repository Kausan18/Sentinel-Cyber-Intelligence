...
# ─── CHART THEME ─────────────────────────────────────────────────────────────
# Shared plotly layout settings for dark theme
# We define this once and reuse it across all charts
CHART_LAYOUT = dict(
    paper_bgcolor="#1E293B",   # chart background
    plot_bgcolor="#1E293B",    # plot area background
...
# Risk level colours — used consistently across all charts
RISK_COLORS = {
    "Critical": "#EF4444",
    "High":     "#F97316",
    "Medium":   "#F59E0B",
...
@st.cache_data(ttl=60)
def fetch_risk_summary():
    """Fetch top 10 highest risk assets."""
    try:
        response = requests.get(f"{API}/risk-summary", timeout=30)
        if response.status_code == 200:
...
def fetch_stats():
    ...
    try:
        response = requests.get(f"{API}/stats", timeout=10)
        if response.status_code == 200:
...
# ─── FETCH ALL DATA ───────────────────────────────────────────────────────────
assets, err1      = fetch_all_assets()
top_assets, err2  = fetch_risk_summary()
stats, err3       = fetch_stats()
...
# Convert assets list to DataFrame for easier manipulation
...
# ── Chart 1: Top 10 Highest Risk Assets (horizontal bar) ─────────────────────
with chart_col1:
    ...
    if top_assets:
        ...
        # Map each asset's risk level to a colour
        top_df["color"] = top_df["risk_level"].map(RISK_COLORS).fillna("#64748B")
 
        fig_bar = go.Figure()
 
        # Add one bar trace per risk level so the legend shows correctly
        ...
        fig_bar.update_layout(
            **CHART_LAYOUT,
            ...
            yaxis=dict(
                title="",
                color="#94A3B8",
                categoryorder="total ascending"  # highest at top
            ),
            barmode="overlay",
        )
...
# ── Chart 3: Risk Level by Environment (stacked bar) ─────────────────────────
with env_col1:
    st.markdown("#### Risk by Environment")
 
    # Group by environment and risk level
    # This creates a count of each risk level per environment
    env_risk = (
...
# ── Chart 4: Internet Exposed vs Internal ─────────────────────────────────────
with env_col2:
    st.markdown("#### Internet Exposed vs Internal")
 
    # Count exposed vs not exposed
...
fig_hist = px.histogram(
    df,
    x="risk_score",
    nbins=20,               # 20 buckets across 0-100
...
with crit_col2:
    # Average risk score per asset type
    type_risk = (
